...
export const getAppointedPatient = (keywork="", currentPage =1) => async (dispatch) => {
  try {
    dispatch({
    ...
    let link = `/api/dash/appointment?page=${currentPage}`;
 ...
 export const appointment = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(`/api/dash/appointment`, userData, config);
...
export const newPrescription = (reviewData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(`/api/dash/presc`, reviewData, config);
